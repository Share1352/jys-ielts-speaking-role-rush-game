...
export function TeacherPage({ socket, roomCode, hostToken }: { socket: Socket; roomCode: string; hostToken: string }) {
  ...
  return <main className='app-shell stack-md'>
    ...
    <section className='panel'>
      ...
      {(phase === 'ready_to_speak' || phase === 'speaker_selection') && <button className='btn' onClick={() => run('teacher:spinSpeaker')}>Spin Speaker Wheel</button>}
      {phase === 'speaking' && <>
        <button className='btn' disabled={Boolean(round?.speakerTimer?.running)} onClick={() => run('teacher:startSpeakerTimer')}>Start Speaking Timer</button>
      ...
      {phase === 'speaker_finished' && <>
        ...
        <button className='btn' onClick={() => run('teacher:revealGuesses')}>Reveal Guesses</button>
      </>}
      {phase === 'guesses_revealed' && <button className='btn' onClick={() => run('teacher:enterScoring')}>Enter Scoring</button>}
      {phase === 'scoring' && <button className='btn' onClick={() => run('teacher:revealSecret')}>Reveal Secret</button>}
      {phase === 'secret_revealed' && <button className='btn' onClick={() => run('teacher:nextSpeaker')}>Next Speaker / Finish Round</button>}
    ...
    {phase === 'scoring' && <section style={{ marginTop: 16 }}>
      <h2>Score guesses</h2>
      {(round?.guesses ?? []).map((g: any) => <div className='row' key={`${g.guesserPlayerId}-${g.targetSpeakerId}`}>
      ...
      {round?.currentSpeakerId && bonusCategories.map((b) => <button className='btn' key={b.id} onClick={() => run('teacher:setSpeakerBonus', { speakerId: round.currentSpeakerId, category: b.id })}>{b.label}</button>)}
    </section>}
  </main>;
}
